...
// Assignment: Program Example 
// File:       Records1.cpp
// Purpose:    Reads magazine sales records, calculates commission rate and commission
//             for each record and writes new file with commission information.
//             If number of subscriptions sold is less than 500, the commission rate is 0.50
//             per subscription; otherwise the commission rate is 0.60 per subscription.
//             Illustrates records, file I/O, memory allocation and template functions.
// Input:      Input file name ( sales.txt ) and output file name. Records read from file.
...
const int RATE_CHANGE_POINT = 10;  // Quantity sold value at which commision rate changes
const double RATE1 = 0.10;          // Regular commission per subscription sold 
const double RATE2 = 1.00;          // Premium commission per subscription sold
...
{  if (quantity_sold < RATE_CHANGE_POINT)
	  purchase_total = RATE1 * quantity_sold * price;
   else
